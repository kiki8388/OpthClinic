...
const DisplayDirectors = ({ directors }) => {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`https://localhost:44345/api/director/directors/${id}/delete`, { headers: auth })
      refreshPage();
    } catch (error) {
      if (error.response) {
      ...
      {cookies.jwt &&
        <>
                ...
                <TableBody>
                  {directors.map((director, index) => (
                    <TableRow key={index}>
                      ...
                      <TableCell align="center">{director.firstName}</TableCell>
                      <TableCell align="center">{director.lastName}</TableCell>
                      <TableCell align="center">{director.email}</TableCell>
                      <TableCell style={{ width: "10%" }} align="center">
                        <Link to={`/${path[1]}/directors/${director.id}`}>
          ...
          <div className='registerNewUserButton'>
            <Link style={{ width: "30%" }} to={`/${path[1]}/directors/register`}><Button variant="contained">Register new director</Button></Link>
          </div>
        </>
      }
      {!cookies.jwt &&
...
export default DisplayDirectors