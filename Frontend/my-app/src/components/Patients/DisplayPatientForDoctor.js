...
import NavBar from '../NavBar/NavBarDoctor';
import { useCookies } from 'react-cookie';
import AuthProvider from '../Auth/AuthProvider';

const DisplayPatientForDoctor = (patients) => {
    const { id } = useParams();
    ...
    const handleAssignPatient = async (doctorId, patientId) => {
        try {
            await axios.post(`https://localhost:44345/api/doctor/${doctorId}/patients/${patientId}/assign_patient`, { headers: auth })
            refreshPage();
        } catch (error) {
            if (error.response) {
                console.log(error.response.data);
                                    ...
                                    <Table>
                                        ...
                                        <TableBody>
                                            ...
                                            <TableRow>
                                                ...
                                                <TableCell align='center' style={{ width: "33.33%" }}>
                                                    <Button style={{ backgroundColor: "#1D3557" }} variant="contained" onClick={() => handleAssignPatient(doctor.id, patient.id)}>Assign patient</Button>
                                                </TableCell>
                                            </TableRow>
                                        </TableBody>
                                    </Table>
...
export default DisplayPatientForDoctor