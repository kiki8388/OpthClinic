...
const PatientProfileVisits = (patient) => {
    ...
    useEffect(() => {
        ...
        const fetchVisits = async () => {
            try {
                const response = await axios.get(`https://localhost:44345/api/${patient.patient.id}/all_visits`, { headers: auth });
            ...
            {cookies.jwt &&
                <>
                                ...
                                <TableHead style={{ backgroundColor: "#457B9D" }}>
                                    <TableRow style={{ backgroundColor: "#457B9D" }}>
                                        <TableCell align='center' colSpan={7}>
                                            <h3>PATIENT {patientData.firstName} {patientData.lastName} VISITS</h3>
                                        </TableCell>
                                    </TableRow>
                                </TableHead>
                                <TableHead>
                                ...
                                <TableBody>
                                    {visits.map((visit, index) => (
                                        <TableRow key={index}>
                                            ...
                                            <TableCell align="center">{visit.doctorFirstName} {visit.doctorLastName}</TableCell>
                    ...
                    <div className='registerNewUserButton'>
                        <Link style={{ width: "30%" }} to={`/${path[1]}/Profile/visits/create`}><Button variant="contained">Create new visit</Button></Link>
                    </div>
...
export default PatientProfileVisits