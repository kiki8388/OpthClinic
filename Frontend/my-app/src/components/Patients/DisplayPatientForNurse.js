...
const DisplayPatientForNurse = (patients) => {
    const { id } = useParams();
    ...
    const handleEditMedicalHistory = async (id) => {
        const newPatient = { patientData };
        try {
            await axios.put(`https://localhost:44345/api/nurse/patients/${id}/edit_medical_history`, newPatient.patientData, { headers: auth })
            setPatientData({
                name: '',
                description: '',
                distReSphere: '',
...
export default DisplayPatientForNurse